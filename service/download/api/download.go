...
import (
	...
	"DStorage/common"
	cfg "DStorage/config"
	dbl "DStorage/service/dbproxy/client"
	"DStorage/store/oss"
	// dlcfg "DStorage/service/download/config"
)
...
func DownloadURLHandler(c *gin.Context) {
	...
	// 从文件表查找记录
	dbResp, err := dbl.GetFileMeta(filehash)
	if err != nil {
		c.JSON(
			http.StatusOK,
			gin.H{
	...
	tblFile := dbl.ToTableFile(dbResp.Data)
...
func DownloadHandler(c *gin.Context) {
	...
	username := c.Request.FormValue("username")
	// TODO: 处理异常情况
	fResp, ferr := dbl.GetFileMeta(fsha1)
	ufResp, uferr := dbl.QueryUserFileMeta(username, fsha1)
	...
	uniqFile := dbl.ToTableFile(fResp.Data)
	userFile := dbl.ToTableUserFile(ufResp.Data)
