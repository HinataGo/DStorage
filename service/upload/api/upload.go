...
import (
	...
	cmn "DStorage/common"
	cfg "DStorage/config"
	"DStorage/service/dbproxy/client"
	"DStorage/service/dbproxy/orm"
	"DStorage/service/mq"
	"DStorage/store/oss"
	"DStorage/util"
)
...
func DoUploadHandler(c *gin.Context) {
	...
	// 6.  更新文件表记录
	_, err = client.OnFileUploadFinished(fileMeta)
	if err != nil {
		errCode = -6
		return
	}
	...
	upRes, err := client.OnUserFileUploadFinished(username, fileMeta)
...
func TryFastUploadHandler(c *gin.Context) {
	...
	fileMetaResp, err := client.GetFileMeta(filehash)
	if err != nil {
		log.Println(err.Error())
	...
	fmeta := client.TableFileToFileMeta(fileMetaResp.Data.(orm.TableFile))
	fmeta.FileName = filename
	upRes, err := client.OnUserFileUploadFinished(username, fmeta)
	if err == nil && upRes.Suc {
		resp := util.RespMsg{
			Code: 0,
			Msg:  "秒传成功",
