...
import (
	...
	mydb "DStorage/service/dbproxy/conn"
)
...
func DeleteUserFile(username, filehash string) (res ExecResult) {
	stmt, err := mydb.DBConn().Prepare(
		"update storage_user_file set status=2 where user_name=? and file_sha1=? limit 1")
	if err != nil {
		log.Println(err.Error())
		res.Suc = false
		res.Msg = err.Error()
...
func RenameFileName(username, filehash, filename string) (res ExecResult) {
	stmt, err := mydb.DBConn().Prepare(
		"update storage_user_file set file_name=? where user_name=? and file_sha1=? limit 1")
	if err != nil {
		log.Println(err.Error())
		res.Suc = false
		res.Msg = err.Error()
...
func QueryUserFileMeta(username string, filehash string) (res ExecResult) {
	stmt, err := mydb.DBConn().Prepare(
		"select file_sha1,file_name,file_size,upload_at," +
			"last_update from storage_user_file where user_name=? and file_sha1=?  limit 1")
	if err != nil {
		res.Suc = false
		res.Msg = err.Error()
		return
